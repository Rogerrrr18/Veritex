...
class QwenAdapter(BaseLLMAdapter):
    ...
    async def process_langchain_messages(self, messages: List[BaseMessage]) -> str:
        ...
        try:
            ...
            print(f"Qwen LangChain消息处理错误: {e}")
            import traceback
            print(f"详细错误信息:")
            traceback.print_exc()
...
async def test_qwen_adapter():
    ...
    try:
        ...
        print(f"❌ Qwen适配器测试失败: {e}")
        print("💡 请检查QWEN_API_KEY配置和网络连接")

if __name__ == "__main__":
    import asyncio
    asyncio.run(test_qwen_adapter())