...
class ClaudeAdapter(BaseLLMAdapter):
    ...
    def _convert_to_claude_format(self, messages: List[Dict[str, str]]) -> Dict:
        """将标准消息格式转换为Claude API格式"""
        system_prompt = ""
        claude_messages = []
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        if not self.config.api_key:
            raise ValueError("CLAUDE_API_KEY 未设置，请检查 .env 文件")
        
        # 转换为Claude API格式
        payload = self._convert_to_claude_format(messages)
        ...
        try:
            ...
            if "content" in result and len(result["content"]) > 0:
                ...
                print(f"Claude API响应格式异常: {result}")
                return None
    ...
    async def process_langchain_messages(self, messages: List[BaseMessage]) -> str:
        ...
        try:
            ...
            print(f"Claude LangChain消息处理错误: {e}")
            import traceback
            print(f"详细错误信息:")
            traceback.print_exc()
...
async def test_claude_adapter():
    ...
    try:
        ...
        if not config_manager.is_model_available("claude"):
            print("❌ Claude模型配置不可用，跳过测试")
            return
            
        config = config_manager._configs["claude"]
        
        # 创建适配器
        async with ClaudeAdapter(config) as adapter:
            # 测试简单聊天
        ...
        print(f"❌ Claude适配器测试失败: {e}")
        print("💡 请检查CLAUDE_API_KEY配置和网络连接")
