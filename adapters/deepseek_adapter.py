...
class DeepSeekAdapter(BaseLLMAdapter):
    ...
    async def process_langchain_messages(self, messages: List[BaseMessage]) -> str:
        ...
        try:
            ...
            print(f"DeepSeek LangChain消息处理错误: {e}")
            import traceback
            print(f"详细错误信息:")
            traceback.print_exc()
...
async def test_deepseek_adapter():
    ...
    try:
        ...
        if not config_manager.is_model_available("deepseek"):
            print("❌ DeepSeek模型配置不可用，跳过测试")
            return
            
        config = config_manager._configs["deepseek"]
        
        # 创建适配器
        async with DeepSeekAdapter(config) as adapter:
            # 测试简单聊天
            response = await adapter.simple_chat("你好，请简单介绍一下自己")
        ...
        print("💡 请检查DEEPSEEK_API_KEY配置和网络连接")
